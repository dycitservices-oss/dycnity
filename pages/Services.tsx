...
const ServiceDetailCard: React.FC<{
  ...
  path?: string;
}> = ({ title, items, icon, colorClass, darkColorClass, path }) => (
  <motion.div
    variants={{
  ...
  >
    <div
      className={`w-14 h-14 ${colorClass} ${darkColorClass} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform`}
    >
      {icon}
    </div>
    ...
    <div className="mt-8 pt-6 border-t border-slate-50 dark:border-slate-700">
      {path ? (
        <Link to={path} className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-semibold text-sm hover:gap-3 transition-all">
...
const Services: React.FC = () => {
  const allServices = [
    {
      title: "End-to-End SWIFT Services",
      icon: (
        <Settings
      ...
      items: [
        "SWIFT MT/MX messaging implementation",
        "gpi integration & tracking",
        "Connectivity (Alliance Access / Alliance Lite2)",
        "Payment reconciliation & formats conversion",
        "Testing, certification & compliance support",
      ],
    },
